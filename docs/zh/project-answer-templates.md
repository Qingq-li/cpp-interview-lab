...
这份文档把语言知识点和项目表达连接起来。每张卡都按“回答目标、常见坑、模板示例、拆解”组织。

## 1. 面试官真正想听的不是“背答案”

### 核心答案

项目回答不要只背 C++ 概念，要把背景、问题、方案、取舍、结果和反思串起来。核心是证明你理解工程约束，而不是只会说 API 名字。

### English explanation

In a project interview, I would connect the technical choice to context, tradeoffs, measurable results, and lessons learned.

### 错误回答示例

- “我们项目里用了很多 C++ 特性，但说不清为什么”
- “只讲结论，不讲背景、约束和取舍”
- “夸大个人贡献或编造量化结果”
...
回答时按“背景 -> 问题 -> 方案 -> 为什么 -> 结果 -> 反思”组织。没有精确数字时，可以说清观察指标、对比方式和你能确认的改进范围，不要硬编数据。

### 深入解释

- 技术点要落到业务约束或工程目标上，例如性能、稳定性、可维护性、生命周期、并发正确性
- tradeoff 比单纯结论更重要，例如性能和复杂度、吞吐和延迟、共享所有权和生命周期清晰度
- 项目表达要避免“我用了某技术所以高级”，要说明“为什么这个场景需要它”
...
- 再指出方案选择背后的约束和替代方案
- 最后补充结果、边界和下一步改进
- 如果被追问技术细节，再回到对应 flashcard 的核心概念解释

### 高频坑

- 不要只说“用了智能指针/线程池/STL”，要说为什么需要
- 不要把团队成果全部说成个人独立完成
- 不要没有数据却硬编性能提升比例

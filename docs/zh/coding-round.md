...
这一篇整理高频手写题。每题按“短答案、边界、代码、讲解、追问”组织，重点是写得正确、解释得清楚，而不是只背模板。

## 1. 反转字符串

### 核心答案

给定一个字符串，将其原地反转。 核心思路是先选对数据结构或指针移动方式，再明确边界条件和复杂度。
...
工程里我会把这类手写题当作基础数据结构和边界处理能力训练：先保证可读、可测、边界明确，再考虑性能优化。真实项目中同类逻辑还要补单元测试和异常输入处理。
...
- 先说明输入为空、单元素、重复值等边界怎么处理
- 再说明循环不变量：每一步已经处理了哪一部分，未处理部分在哪里
- 最后说明时间复杂度和额外空间复杂度
- 面试中如果代码较长，优先解释关键状态变量和更新顺序
...
判断一个字符串是否为回文串。 核心思路是先选对数据结构或指针移动方式，再明确边界条件和复杂度。
...
给定单链表头节点，返回反转后的头节点。 核心思路是先选对数据结构或指针移动方式，再明确边界条件和复杂度。
...
返回单链表的中间节点。 核心思路是先选对数据结构或指针移动方式，再明确边界条件和复杂度。
...
输入两个递增链表，合并成一个递增链表并返回头节点。 核心思路是先选对数据结构或指针移动方式，再明确边界条件和复杂度。
...
设计一个支持 `get` 和 `put` 的 LRU，要求平均 O(1)。 核心思路是先选对数据结构或指针移动方式，再明确边界条件和复杂度。
...
使用两个栈实现先进先出的队列。 核心思路是先选对数据结构或指针移动方式，再明确边界条件和复杂度。
...
设计一个栈，支持 `push`、`pop`、`top` 和 `getMin`，都要求 O(1)。 核心思路是先选对数据结构或指针移动方式，再明确边界条件和复杂度。
...
设计一个多线程可用的队列，支持生产者和消费者并发访问。 核心思路是先选对数据结构或指针移动方式，再明确边界条件和复杂度。
...
如果让你实现一个简单线程池，你会怎么设计任务队列？ 核心思路是先选对数据结构或指针移动方式，再明确边界条件和复杂度。

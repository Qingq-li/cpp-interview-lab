...
3. 接着看错误回答和面试官关注点，避免只背定义。
4. 最后用示例代码和中文注释把概念落到可运行行为上。
...
- A pointer is a variable that holds an address. It can be null and can be reseated to another object
- A reference is an alias for an object and cannot be rebound after initialization.
...
- Stack objects usually have automatic storage duration: they are created and destroyed by scope.
- Heap objects are dynamically allocated, so their lifetime can be decoupled from a single scope.
- In modern C++, heap objects should usually be managed by RAII types such as smart pointers, not by raw `new/delete`.
...
RAII ties resource ownership to object lifetime. Resources are acquired in construction and released automatically when the object is destroyed.
...
- 资源释放要交给 RAII 对象，不要散落在多条 return 路径里
- 能用标准库成员表达资源时，优先 Rule of Zero
- 手写资源类时必须同时考虑拷贝、移动、析构和异常路径
...
- Overriding: a derived class provides its own implementation of a base class virtual function
...
- 多态接口通常通过基类指针或引用使用，避免对象切片
- 作为多态基类时要考虑虚析构
- 不要为了复用几行代码就设计很深的继承层级
...
- A virtual function may provide a default implementation
- A pure virtual function makes the class abstract and requires concrete derived classes to provide an implementation
...
- 不要只背复杂度表，还要考虑内存布局和迭代器失效
- 默认容器通常从 vector 或 unordered_map 开始，再按访问模式修正
- 保存迭代器、引用或指针后继续插入删除时要特别小心
...
Because it stores contiguously, is cache-friendly, has fast random access, and works well with STL algorithms.
...
Because raw `new/delete` easily leads to leaks, missed exception paths, and unclear ownership. Modern C++ uses RAII types to make resource ownership explicit.
...
- `shared_ptr` expresses shared ownership, can be copied, and internally manages the lifetime through reference counting
...
- `std::vector` is good at tail insertion and random access, and elements are stored contiguously
- `std::deque` supports efficient insertion and deletion at both ends and also supports random access, but its storage is usually segmented rather than one contiguous block.
...
- Use `vector` as the default choice
...
- 先找共享可变状态，再选择同步工具
- 不要把能跑几次当成线程安全证明
- 业务代码优先选择容易证明正确的锁和 RAII，再考虑 atomic 优化
...
Since C++11, you can use `std::thread` to create a thread and let a function or callable object run in a new thread.
...
- `detach()` means to let the thread run independently after it is detached from the current `std::thread` object
...
Because when multiple threads access the same mutable data at the same time, if there is no synchronization mechanism, race conditions may occur, resulting in uncertain results.
...
`enum class` is a strongly typed enumeration, which is safer than traditional `enum`. It prevents enumerator names from polluting the surrounding scope and avoids implicit conversion to integers.
...
- `priority_queue`: pop elements according to priority, default maximum heap
...
- `dynamic_cast`: mainly used for safe downcasting in inheritance system
...
- 预处理处理 `#include`、`#define`、条件编译等文本级动作
- 编译把源代码翻译成目标文件，并做语法、类型和语义检查
- 链接把多个目标文件和库合成最终可执行文件

### English explanation

In an English interview, I would answer it like this:

Preprocessing handles includes and macros, compilation turns source files into object files, and linking resolves symbols across object files and libraries to produce the final executable.
...
- 你是否能区分 compile error 和 link error
- 你是否理解声明、定义、头文件和库之间的关系

### 项目里怎么说

在排查构建问题时，我会先看错误发生在编译阶段还是链接阶段：类型不匹配通常是编译错误，未定义引用通常是链接错误。

### 深入解释

- 预处理结果仍然是 C++ 源文本
- 每个 `.cpp` 通常形成一个翻译单元
- 链接器关心符号定义是否存在、是否重复、是否 ABI 匹配
...
void printName(); // 声明：告诉编译器函数存在

int main() {
    printName();
}

void printName() { // 定义：链接时需要找到真正实现
    std::cout << APP_NAME << '\n';
}
```
...
- `#define APP_NAME` 在预处理阶段被替换
- `void printName();` 是声明，让 `main` 可以通过编译
- 真正的函数体是定义，链接阶段需要有且只有一个合适定义
### 高频坑
...
Scope is about where a name can be used, lifetime is about when an object is alive, and storage duration is about how long the underlying storage exists.
...
- 你是否能区分名字可见性和对象是否还活着
- 你是否理解局部变量、静态变量和动态分配对象的差别

### 项目里怎么说

我会用作用域控制可见性，用 RAII 控制生命周期，只有在对象必须跨作用域存在时才考虑动态存储或更长存储期。

### 深入解释

- 局部自动对象通常离开块作用域即销毁
- `static` 局部变量的名字只在函数内可见，但对象存储持续到程序结束
- 动态分配对象的生命周期由显式释放或智能指针控制
...
int nextId() {
    static int id = 0; // 静态存储期，只初始化一次
    return ++id;
}

int main() {
    int local = nextId(); // 自动存储期，离开 main 时销毁
    std::cout << local << ' ' << nextId() << '\n';
}
```
...
- `id` 的名字只在 `nextId` 内可见
- `id` 的对象本身不会在函数返回时销毁
- `local` 是普通局部对象，生命周期和 `main` 的块作用域一致
### 高频坑
...
- namespace 用来组织名字，避免不同库或模块之间命名冲突
- 它不会改变对象生命周期，也不是访问权限控制
- 工程里通常避免在头文件里写 `using namespace`

### English explanation

In an English interview, I would answer it like this:

A namespace groups names and prevents name collisions. It is an organization tool, not an ownership or access-control mechanism.

### 错误回答示例

- “namespace 会让代码更快”
- “头文件里 `using namespace std;` 没问题”
- “namespace 和 class 的作用一样”
...
在公共头文件里我会使用明确的命名空间和限定名，避免把 `using namespace` 泄漏给调用方。

### 深入解释

- 命名空间可以嵌套
- 同名 namespace 可以在多个文件中继续扩展
- 命名空间设计会影响 API 可读性和符号组织
...
- `math::add` 明确表示函数属于 `math` 命名空间
- 调用处使用限定名，避免和其他 `add` 冲突
- 这个机制只管理名字，不管理内存
### 高频坑
...
- 成员初始化列表直接初始化成员
- `const` 成员、引用成员和没有默认构造的成员必须在初始化列表中初始化
- 它通常比先默认构造再赋值更清晰、更高效
...
A member initializer list initializes members directly before the constructor body runs. It is required for const members, reference members, and members without default constructors.
...
- 你是否知道成员真正初始化发生在构造函数体之前
- 你是否知道初始化顺序由成员声明顺序决定

### 项目里怎么说

我会默认使用成员初始化列表，尤其是资源类、不可变配置和依赖注入对象，这样对象构造过程更明确。

### 深入解释

- 构造函数体执行时，成员已经完成初始化
- 初始化列表顺序不决定真实顺序，类中声明顺序才决定
- 初始化列表能避免不必要的默认构造加赋值
...
- `id_(id)` 在对象构造阶段初始化 `const` 成员
- `name_(std::move(name))` 直接构造字符串成员
- 构造函数体为空也不影响成员已经初始化完成
### 高频坑
...
## 66. 默认构造、默认成员初始化、委托构造是什么？

### 核心答案

- 默认构造函数是不需要实参即可调用的构造函数
- 默认成员初始化给成员提供类内初始值
- 委托构造让一个构造函数调用同类的另一个构造函数

### English explanation

In an English interview, I would answer it like this:

A default constructor can be called without arguments, default member initializers give members in-class defaults, and delegating constructors reuse another constructor in the same class.
...
- 你是否能写出更少重复、更安全的初始化代码
- 你是否理解默认值和构造重用

### 项目里怎么说

我会用默认成员初始化表达稳定默认值，用委托构造减少重复初始化逻辑，避免不同构造函数把对象带到不同状态。

### 深入解释

- 只要用户声明某些构造函数，默认构造不一定自动生成
- 构造函数初始化列表可以覆盖默认成员初始化
- 委托构造只能委托给同类一个目标构造函数
...
class Task {
public:
    Task() : Task("untitled") {} // 委托构造
    explicit Task(std::string name) : name_(std::move(name)) {}

private:
    std::string name_;
    int priority_ = 0; // 默认成员初始化
};
```

### 代码讲解

- `priority_ = 0` 是成员自己的默认值
- `Task()` 把构造逻辑委托给 `Task(std::string)`
- `explicit` 防止字符串参数带来意外隐式转换
### 高频坑
...
## 67. `std::string` 和 C 风格字符串有什么区别？

### 核心答案

- `std::string` 拥有并管理字符串内存
- C 风格字符串通常是以 `\0` 结尾的字符数组
- 现代 C++ 接口优先用 `std::string` 或 `std::string_view` 表达语义

### English explanation

In an English interview, I would answer it like this:

`std::string` owns and manages character storage, while a C-style string is usually a null-terminated character array. `std::string` is safer for most application-level code.

### 错误回答示例

- “`char*` 和 `std::string` 只是写法不同”
- “C 字符串都知道自己的长度”
- “`c_str()` 返回的指针可以永久保存”
...
业务代码里我通常用 `std::string` 持有文本；只有和 C API 交互时才临时用 `c_str()`。

### 深入解释

- `std::string::size()` 不依赖扫描结尾
- C 字符串函数通常需要找到 `\0`
- 修改 string 后，之前拿到的 `c_str()` 指针可能不再可靠
...
int main() {
    std::string s = "hello";
    const char* p = s.c_str(); // 临时给 C API 使用
...
## 68. `std::vector` 常见操作和容量变化怎么理解？

### 核心答案

- `size()` 是元素个数，`capacity()` 是当前已分配容量
- 尾部 `push_back` 平均 O(1)，扩容时可能整体搬迁元素
- 扩容会使原来的指针、引用、迭代器失效
...
For `std::vector`, size is the number of elements and capacity is the allocated storage. When capacity grows, elements may be moved to new storage and old iterators or pointers become invalid.

### 错误回答示例

- “`size` 和 `capacity` 是一回事”
- “`push_back` 永远不会影响旧引用”
- “`reserve` 会创建元素”
...
如果我能预估元素数量，会先 `reserve()` 降低扩容次数；但不会用 `reserve()` 当作创建元素。

### 深入解释

- `reserve(n)` 只保证容量至少为 n
- `resize(n)` 会改变元素个数
- 扩容策略由实现决定，不能依赖固定倍数
...
- `reserve(3)` 后 `size()` 仍然是 0
- 两次 `push_back` 后元素个数是 2
- 容量至少为 3，但具体值由实现决定
### 高频坑
...
## 69. range-based for 里值拷贝和引用写法有什么区别？

### 核心答案

- `for (auto x : c)` 会拷贝元素
- `for (auto& x : c)` 可以修改原元素
- `for (const auto& x : c)` 避免拷贝并保持只读

### English explanation

In an English interview, I would answer it like this:

In a range-based for loop, `auto x` copies each element, `auto& x` iterates by mutable reference, and `const auto& x` avoids copies while keeping elements read-only.
...
遍历大对象容器时我会优先写 `const auto&`；只有明确要修改元素时才写 `auto&`。
...
Default arguments belong to function declarations and are used at the call site. In the same scope, a default value should be specified only once.
...
我会把公共函数默认参数放在头文件声明里，源文件定义不重复写，减少维护风险。

### 深入解释

- 默认参数不是函数重载
- 默认参数值由静态类型和可见声明决定
- 虚函数默认参数不会像虚函数调用一样动态分派
...
void log(int level = 1); // 默认参数写在声明处
...
## 71. operator overloading 基础认知是什么？

### 核心答案

- 运算符重载让自定义类型支持类似内置类型的表达方式
- 它应保持直觉语义，不应滥用
- 不能创建新运算符，也不能改变运算符优先级

### English explanation

In an English interview, I would answer it like this:

Operator overloading lets user-defined types support natural expressions, but it should preserve intuitive semantics. It cannot create new operators or change precedence.
...
我只会在语义非常自然时重载运算符，比如值类型的比较、加法或输出；业务动作更适合普通命名函数。

### 深入解释

- `operator==` 应表达相等性
- `operator<<` 通常写成非成员函数以支持左操作数是 stream
- 重载不应制造意外副作用
...
## 72. 异常 `try/catch/throw` 基础怎么理解？

### 核心答案

- `throw` 抛出异常，`try` 包围可能失败的代码，`catch` 处理异常
- 异常会沿调用栈向外传播，直到被匹配的 handler 捕获
- 析构函数和 RAII 对异常安全非常重要

### English explanation

In an English interview, I would answer it like this:

`throw` reports an exceptional failure, `try` marks code that may throw, and `catch` handles matching exceptions. Stack unwinding makes RAII important for cleanup.
...
我会用异常表达无法在当前层处理的错误，同时确保资源交给 RAII 对象管理，避免异常路径泄漏。
...
- `parse(false)` 抛出 `runtime_error`
- `catch (const std::exception&)` 捕获标准异常基类
- `e.what()` 提供错误信息
### 高频坑
...
## 73. `std::sort` 自定义比较器怎么写？

### 核心答案

- 比较器应返回第一个元素是否严格排在第二个元素前
- 比较器必须满足严格弱序
- 不要在比较器里写不稳定的副作用逻辑
...
A `std::sort` comparator should implement a strict weak ordering: it returns whether the first argument should come before the second.
...
- 你是否知道严格弱序
- 你是否知道需要稳定顺序时应考虑 `stable_sort`

### 项目里怎么说

我会让比较器只描述排序规则，不混入业务副作用；如果需要多字段排序，会明确写出 tie-breaker。
...
int main() {
    std::vector<User> users{{"a", 90}, {"b", 80}, {"c", 90}};
    std::sort(users.begin(), users.end(), [](const User& lhs, const User& rhs) {
        return lhs.score > rhs.score; // 分数高的排前面
    });
    std::cout << users.front().name << '\n';
}
```
...
## 74. 返回引用或指针时有什么生命周期风险？

### 核心答案

- 不能返回指向局部自动变量的引用或指针
- 返回引用通常要求被引用对象比调用方使用时间更长
- 不确定所有权时优先返回值或智能指针
...
Returning a reference or pointer is safe only if the referenced object outlives the caller's use. Never return references or pointers to local automatic variables.
...
- 你是否理解 dangling reference/pointer
- 你是否知道现代 C++ 返回值优化很强

### 项目里怎么说

接口设计里我会优先返回值；只有对象由容器、成员或上层明确拥有时，才返回引用或指针作为非拥有访问。
...
const std::string& bad() {
    ...
    return s; // 错误：返回局部对象引用
}

std::string good() {
    return "value"; // 返回值通常很安全，也可能被优化
}
```
...
- `bad()` 返回后 `s` 已经销毁
- 继续使用 `bad()` 的返回引用会产生未定义行为
- `good()` 返回值由调用方接收，生命周期清晰
### 高频坑
...
- 头文件放类声明、接口、必要的类型定义
- 源文件放非模板、非 inline 函数实现
- 头文件应尽量减少依赖，使用 include guard 或 `#pragma once`

### English explanation

In an English interview, I would answer it like this:

A header should expose declarations and the public interface, while a source file should contain implementation details for non-template, non-inline functions.
...
小型类我会把稳定接口放头文件，把实现细节放 `.cpp`，减少重新编译范围，也让调用方只依赖必要信息。
...
- 头文件声明 `User` 的公共接口
- 源文件实现构造函数和成员函数
- 实际代码中 `User.cpp` 还需要包含 `User.h` 和 `<utility>`
### 高频坑
